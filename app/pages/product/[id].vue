...
function handleAddToCart() {
  ...
  const isBookable = product.value.productType?.toLowerCase() === "bookable";

  // Check if product already exists in cart
  const existing = cart.value.find((p) => p.id === route.params.id);

  // Prevent adding more than one bookable product
  ...
  addToCart({
    ...
    price: product.value.variants?.edges?.[0]?.node?.price?.amount || "0",
    compareAtPrice:
      product.value.variants?.edges?.[0]?.node?.compareAtPrice?.amount || "0",
    image: product.value.images?.edges?.[0]?.node?.url || "",
    quantity: 1,
    variantId,
    ...
    <div class="open-hero_rhs">
      <div class="rhs-content_holder">
        ...
        <div class="web-heading-4 uc-white">
          ${{
            product?.productType?.toLowerCase() === "bookable"
        ...
        <div class="cart-content_holder">
          <div v-if="product?.productType.toLowerCase() === 'bookable'">
            <div
              v-if="!currentItem"
              class="book-now-btn_full"
              @click="handleAddToCart"
            >
              Book appointment
            </div>
            <div v-else class="added-btn_full">Added to cart</div>
          </div>
          ...
          <div v-else class="full-width">
            ...
            <div v-else class="cart-counter-product_brown">
              ...
              <div class="web-body-text uc-white">
