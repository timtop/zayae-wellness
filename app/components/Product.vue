...
interface VariantNode {
  compareAtPrice?: ComparePriceNode | null;
  id: string;
  price: PriceNode;
}
...
function handleAddToCart() {
  ...
  const isBookable =
    props.product.node.productType.toLowerCase() === "bookable";

  // Check if product is already in cart
  const existing = cart.value.find((p) => p.id === props.product.node.handle);

  // If it's bookable and already added, do nothing
  if (isBookable && existing) {
    console.log("Bookable product already in cart");
    return;
  }
  ...
  addToCart({
    ...
    compareAtPrice:
      props.product.node.variants.edges[0]?.node.compareAtPrice?.amount || "0",
    image: props.product.node.images.edges[0]?.node.url || "",
    quantity: 1,
    variantId,
    productType: props.product.node.productType,
  });
}
    ...
    <div class="product-textholder">
      <div class="product-details">
        ...
        <div class="product-price">
          ${{
            product.node.productType.toLowerCase() === "bookable"
              ? product.node.variants.edges[0]?.node.compareAtPrice?.amount
              : product.node.variants.edges[0]?.node.price.amount
          }}
        </div>
      </div>
      <div class="product-typer">
        <div
          v-if="product.node.productType.toLowerCase() === 'bookable'"
          class="book-holder"
        >
          <div
            v-if="quantity === 0"
          ...
          <div v-else class="added-btn">Added to cart</div>
        </div>

        <div v-else class="cart-holder">
          <div
            v-if="quantity === 0"
            class="cart-button"
            @click="handleAddToCart"
          ...
          <div v-else class="cart-counter">
            ...
            <div class="web-body-text uc-brown product-count">
              {{ quantity }}
            </div>
...
.product:hover .book-holder .book-now-btn {
  background: var(--beige--200);
  color: #974619;
  /* border: 1px solid #ffffff; */
}

.product:hover .book-holder .added-btn {
